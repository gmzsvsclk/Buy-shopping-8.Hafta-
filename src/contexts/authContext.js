...
export function AuthContext({ children }) {
  ...
  async function createUser(data) {
    ...
    if (index !== -1) {
      toast.error("Mail adresi zaten var, Tekrar deneyin!");
      return;
    }
  ...
  async function SignIn(data) {
    ...
    if (index === -1) {
      toast.error("Mail adresi geçerli değil");
      return false;
    }
    ...
    if (userList[index].password === data.password) {
      ...
      toast.error("Verilen kullanıcı adı veya parola herhangi bir GSC hesabı ile eşleşmiyor.");
      return false;
    }
  }
